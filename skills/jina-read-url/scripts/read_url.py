...
JINA_READER_ORIGIN = "https://r.jina.ai/"
...
def is_reader_url(url: str) -> bool:
    parsed = urllib.parse.urlsplit(url)
    return parsed.scheme in {"http", "https"} and parsed.netloc == "r.jina.ai"
...
def build_headers(extra_headers: list[tuple[str, str]]) -> dict[str, str]:
    headers = {
        "User-Agent": "jina-read-url-skill/1.0",
    }
...
def build_request(target_url: str, extra_headers: list[tuple[str, str]]) -> urllib.request.Request:
    ...
    if is_reader_url(target_url):
        return urllib.request.Request(target_url, headers=headers, method="GET")
    ...
    return urllib.request.Request(JINA_READER_ORIGIN + target_url, headers=headers, method="GET")
